...
using VirtualWorkFriendBot.Adapters;
using VirtualWorkFriendBot.Authentication;
using VirtualWorkFriendBot.Bots;
using VirtualWorkFriendBot.Controllers;
using VirtualWorkFriendBot.Dialogs;
using VirtualWorkFriendBot.Helpers;
using VirtualWorkFriendBot.Services;


namespace VirtualWorkFriendBot
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            //A - add resource explorer
            //var resourceExplorer = ResourceExplorer.LoadProject(this.HostingEnvironment.ContentRootPath);
            //services.AddSingleton(resourceExplorer);
  

            InitializeCosmosDB(services, settings.CosmosDb);

            // Configure localized responses
            var localizedTemplates = new Dictionary<string, List<string>>();
            var templateFiles = new List<string>() { "Main", "Onboarding" };
            ...
            foreach (var locale in supportedLocales)
            {
                ...
                foreach (var template in templateFiles)
                {
                    ...
                    {
                        localeTemplateFiles.Add(Path.Combine(".", $"Responses/{template}", $"{template}Responses.lg"));
                    }
                    else
                    {
                        localeTemplateFiles.Add(Path.Combine(".", $"Responses/{template}", $"{template}Responses.{locale}.lg"));
                    }
                }
            ...
            services.AddTransient<ChitchatDialog>();
            services.AddTransient<EscalateDialog>();
            services.AddTransient<CancelDialog>();
            services.AddTransient<EntertainDialog>();
            services.AddTransient<StressDialog>();
            services.AddTransient<StressHandlingDialog>();
            services.AddTransient<HighStressHandlingDialog>();
            services.AddTransient<BreatherDialog>();
            services.AddTransient<KnowledgeBaseDialog>();
            services.AddTransient<JournalingDialog>();
            //  services.AddTransient<ComposerDialogs>();

            services.AddTransient<LoginDialog>();
            services.AddScoped<ConfirmPrompt>(  _ => 
                new ConfirmPrompt(
                    nameof(ConfirmPrompt))
                );
            services.AddTransient<OAuthPrompt>(    _ => 
                new OAuthPrompt(
                    nameof(OAuthPrompt), 
                    new OAuthPromptSettings { 
                        ConnectionName = Startup.ConnectionName, 
                        Title = "Sign In",
                        Text = "I'll be working with some of your data so I need you to log in.\nIf a window gives you a code, please let me know the code so I know it's you!"
                    }));
        ...
        private void InitializeCosmosDB(IServiceCollection services, CosmosDbStorageOptions options)
        {
            #region Initialize
            /*
            ...
            */
            ...
            CosmosDBHelper.Initialize(options);
